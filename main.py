import os, sys, time, win32api, winreg, tempfile, tkinter, wget, shutil
...
try:
    ...
    if not os.path.exists("msedgedriver.exe") or getVersion("msedgedriver.exe") != edge_version:
        ...
        shutil.unpack_archive(tempdir + "/edgedriver_win64.zip", "./", "zip")
    ...
    while True:
        ...
        time.sleep(0.5)
        browser.find_element("xpath", "//button[@onclick='saveinfo()']").click()
        time.sleep(0.5)
        browser.get("https://www.ehuixue.cn/index/personal/mystudy")
        if browser.execute_script("return location.href;") == "https://www.ehuixue.cn/index/personal/mystudy":
            if not os.path.exists("userdata") and check_var.get() == True:
                file = open("userdata", "w")
                file.write(f"{username.get()}\n{password.get()}")
                file.close()
            break
        tkinter.messagebox.showwarning("提示", "用户名或密码错误！")
        window.deiconify()
        window.mainloop()
    while True:
        while True:
